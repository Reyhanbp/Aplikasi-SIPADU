...
<div class="card"> <div class="card-header"> <div class="card-title text w-100 d-flex justify-content-between">
    ...
    <span class=" text-muted fw-light">Data Pendatang / </span> Edit Data Pendatang
    </h4>
    <div>
...
<hr>
<form action="{{route('update-pendatang', $pendatang->id)}}" method="POST" enctype="multipart/form-data">
    @csrf
    <div class="card-body">
      <div class="row">
        <div class="col-12">
            <div class="mb-3">
                ...
                <select class="form-control" name="pelapor_id" placeholder="Pilih pelapor_id" id="pelapor_id" required>
                    ...
                    <option value="{{ $data->id }}" @if ($pendatang->pelapor_id === $data->id) selected @endif>
                        {{ $data->name }}
                    </option>
                    @endforeach
                    </select>
        ...
        <div class="col-6">
                ...
                <input type="number" name="NIK" id="NIK" class="form-control" required autoComplete="off" placeholder="NIK"  value="{{$pendatang->NIK}}"/>
            </div>
        </div>
        <div class="col-6">
            <div class="mb-3">
                <label for="name" class="form-label required"> Name </label>
                <input type="text" name="name" id="name" class="form-control" required autoComplete="off" placeholder="Name" value="{{$pendatang->name}}"/>
            </div>
        </div>
        <div class="col-6">
            <div class="mb-3">
                <label for="tgl_datang" class="form-label required"> Tanggal pendatang </label>
                <input type="date" name="tgl_datang" id="tgl_datang" class="form-control" required autoComplete="off" value="{{$pendatang->tgl_datang}}"/>
            </div>
        </div>
        <div class="col-6">
        <div class="mb-2">
