...
@section('content')
<div class="card">
  <form action="{{ route('melahirkan') }}">
    <div class="card-header ">
      <div class="d-flex justify-content-between">
          <div class="card-title text w-100 d-flex justify-content-between">
              <h4 class="fw-bold">
                  <span class=" text-muted fw-light">Index Data Melahirkan / </span> Data Melahirkan
              </h4>
          </div>
           <div class="col-4 ms-md-3 pe-md-3 d-flex align-items-center">
            ...
            <div class="mt-3" >
             <a class="btn btn-3 btn-primary w-200" type="button" style="width: 150px;"  href="{{ route ('tambah-melahirkan') }}">
        ...
        <div class="row">
           <div class="card mb-5">
                <div class="table-responsive">
                    ...
                    <tbody>
                           @foreach ($data as $melahirkan)
                        <tr>
                          <td>{{$loop->iteration }}</td>
                          <td>{{$melahirkan->name }}</td>
                          <td>{{$melahirkan->tgl_lahir }}</td>
                          <td>{{$melahirkan->jenis_kelamin }}</td>
                          <td>{{$melahirkan->Data_kk->no_kk }}</td>


                        <td class="text-center align-middle" >
                            <form action="{{ route ('delete-melahirkan', $melahirkan->id) }}" method="POST">
                                @csrf
                                @method('DELETE')
                                <a class="btn btn-warning" role="button"  href="{{ route ('edit-melahirkan', $melahirkan->id) }}">
