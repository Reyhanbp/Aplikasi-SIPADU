...
<div class="card-body">
<form action="{{ route('indexdatapindah', ['tglawal' => $tglawal, 'tglakhir' => $tglakhir]) }}" method="GET">
    ...
    <h4 class="fw-bold">
        ...
        <div class="card mb-5">
            ...
            <thead>
                <tr>
                    ...
                    <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2">Tanggal Pindah
                    </th>
                    <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2">Sebab Pindah
                    </th>
                </tr>
            </thead>
            <tbody>
                @foreach ($data as $indexdatapindah)
                <tr>
                    <td>{{$loop->iteration }}</td>
                    <td>{{$indexdatapindah->data_penduduk->name }}</td>
                    <td>{{$indexdatapindah->tgl_pindah }}</td>
                    <td>{{$indexdatapindah->sebab_pindah }}</td>
                </tr>
                @endforeach
            </tbody>
            </table>
        ...
        <script>
            function cetak(tglawal, tglakhir) {
                var url = "{{ route('cetakdatapindah', ['tglawal' => 'tglawal', 'tglakhir' => 'tglakhir']) }}";
